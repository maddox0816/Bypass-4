    ...
    return async function onRequest(clientRequest, clientResponse) {
        try {
            ...
            const requestContext = {
                url: urlData.value,
                flags: urlData.flags,
                origin: ((clientRequest.socket.encrypted || config.forceHttps) ? 'https://' : 'http://') + clientRequest.headers.host,
                body: await getChunks(clientRequest),
                headers: { ...clientRequest.headers },
                method: clientRequest.method, 
                rewrite: ctx,
                agent: new ((urlData.value.protocol == 'https:' || config.forceHttps) ? https : http).Agent({
            ...
            ((requestContext.url.protocol == 'https:' || config.forceHttps) ? https : http).request({
